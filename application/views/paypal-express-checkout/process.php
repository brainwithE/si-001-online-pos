...
$paypalmode = ($PayPalMode=='sandbox') ? '.sandbox' : '';
	...
	foreach($this->cart->contents() as $items)
    {
		...
		// $obj = $this->cart_model->getProduct($product_code);

		// $obj = $result->fetch_object();
		
        $paypal_data .= '&L_PAYMENTREQUEST_0_NAME ='.urlencode($items['name']);
        $paypal_data .= '&L_PAYMENTREQUEST_0_NUMBER ='.urlencode($items['product_code']);
        $paypal_data .= '&L_PAYMENTREQUEST_0_AMT ='.urlencode($items['price']);		
		$paypal_data .= '&L_PAYMENTREQUEST_0_QTY ='. urlencode($items['qty']);
		...
		$paypal_product['items'][] = array('itm_name'=>$items['name'],
											'itm_price'=>$items['price'],
											'itm_code'=>$items['product_code'], 
											'itm_qty'=>$items['item_qty']
											);
    }
